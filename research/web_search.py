...
提供心理领域的网络搜索能力，基于 DuckDuckGo Search API (DDGS)
"""
...
class WebSearchTool:
    ...
    async def search(
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                # 在查询中添加心理相关关键词，提高结果相关性
                enhanced_query = f"{query} 心理" if "心理" not in query else query
    ...
    def filter_by_domain(
        ...
        Args:
            results: 搜索结果
            allowed_domains: 允许的域名列表（默认使用心理域名白名单）

        Returns:
            过滤后的结果
        """
        if allowed_domains is None:
            allowed_domains = self.psychology_domains
...
# 便捷函数
async def search_psychology_web(
    query: str,
    max_results: int = 10
) -> List[SearchResult]:
    """
