...
async def generate_test_report(passed: int, failed: int, total: int, context_aware: bool):
    ...
    report = f"""# PsyX Agent Swarm 测试报告
...
async def test_long_term_memory():
    ...
    if not ltm.enabled:
        ...
        print("   配置方法：在项目根目录 .env 中设置 MEM0_API_KEY=...")
...
async def test_singleton_instances():
    """测试 7.3: 单例模式验证"""
    print("\n" + "="*70)
    print("测试 7.3: 单例模式 - PsychologyKnowledgeBase & ShortTermMemory")
    print("="*70)

    # 测试 PsychologyKnowledgeBase 单例
    print("\n🔍 测试 PsychologyKnowledgeBase 单例...")
    from knowledge.milvus_kb import PsychologyKnowledgeBase

    kb1 = PsychologyKnowledgeBase()
    kb1_id = id(kb1)
    print(f"  - 第一次实例化: id={kb1_id}")

    kb2 = PsychologyKnowledgeBase()
    kb2_id = id(kb2)
    print(f"  - 第二次实例化: id={kb2_id}")

    assert kb1 is kb2, "PsychologyKnowledgeBase 应该是单例"
    print(f"✅ PsychologyKnowledgeBase 单例验证通过")
    ...
    print("\n✅ 测试 7.3 通过！")
    print("  ✓ PsychologyKnowledgeBase 单例生效")
...
async def main():
    """运行所有测试"""
    print("\n" + "🧪 "*35)
    print(" "*15 + "PsyX Agent Swarm 完整测试套件")
