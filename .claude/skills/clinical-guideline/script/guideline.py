...
async def clinical_guideline(query: str, max_results: int = 1) -> Dict[str, Any]:
    ...
    # 使用 Milvus 检索心理干预指南
    results = kb.search(
        query=f"{query} 心理干预指南 循证实践 干预方案",
    ...
    if results and results[0]["score"] > 0.1:
        ...
        # 未找到相关内容
        logger.warning(f"No psychological guidelines found in vector DB for {query}")
        return {
            "answer": f"未找到'{query}'的相关心理干预指南，建议使用更具体的议题名称或联系专业机构获取权威指南。",
            "guideline_title": "",
            "organization": "",
            "source": "未找到"
        }
